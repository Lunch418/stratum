...
export function buildMenus(ctx: MenuContext): Menu[] {
  ...
  return [
    { title: 'Файл', items: [
      ...
      { label: 'Параметры печати…', run: () => ctx.open('print'), disabled: noProject },
      { sep: true },
    ...
    { title: 'Моделирование', items: [
      ...
      { label: 'Очистить', sub: [
        ...
        { label: 'Закрыть потоки', run: () => api.event('type=closestreams').then(() => s.showToast('Потоки модели закрыты')) },
        { label: 'Закрыть окна', run: () => api.event('type=closewindows').then(() => s.showToast('Окна модели закрыты')) },
