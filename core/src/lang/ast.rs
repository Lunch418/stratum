...
pub enum UnOp {
    Neg,
    Not,
    /// `~x` — значение, уже присвоенное в этом такте (новая фаза, `push_new`
    /// в байт-коде оригинала). Без тильды читается значение на начало такта.
    /// Имя варианта историческое.
    Old,
}
...
#[derive(Debug, Clone, PartialEq)]
pub enum Stmt {
    /// Метка «дальше идёт оператор со строки N» — для ошибок и отладчика.
    At(u32),
    Declare(Declaration),
