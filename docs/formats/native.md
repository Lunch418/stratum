...
графике листа при загрузке. При экспорте в `.cls` площадка рисуется на
листе как у оригинала — двойная битовая карта из `VARPOINT.DBM` с handle
площадки, а связь через неё — ломаная с handle связи (флаги `0x1800`);
перенесённая площадка тянет за собой концы линий. Так площадки переживают
экспорт и повторный импорт. Байт-код (`0x0d`) не хранится: текст компилируется заново.
